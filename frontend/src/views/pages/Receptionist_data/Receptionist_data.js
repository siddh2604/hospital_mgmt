...
const Page404 = () => {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: "http://localhost:8080/user/get_receptionist_data",
      headers: {
        Authorization: token,
      },
    }).then((response) => {
  ...
  useEffect(() => {
    axios({
      method: "post",
      url: "http://localhost:8080/user/get_receptionist_data_by_doctor_id",
      headers: {
        Authorization: token,
      },
      data: {
  ...
  function handleDelete(id) {
    try {
      fetch('http://localhost:8080/user/delete_receptionist', {
        method: 'POST',
        headers: {
          Authorization: token,
    ...
    <div className="">
      ...
      <div className="wrapper d-flex flex-column min-vh-100 bg-light">
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
          <Link to={`/create_receptionist`}><button class="btn btn-primary me-md-2" type="button">Register Receptionist</button></Link>
        </div>
          <CTable>
            <CTableHead>
              <CTableRow>
                ...
                <CTableRow key={i}>
                  ...
                  <CButtonGroup size="sm" role="group" aria-label="Small button group">
                    <div class="btn-group btn-group-sm" role="group" aria-label="Small button group">
                      <button type="button" class="btn btn-outline-dark"> <Link to={`/receptionist_data/update_receptionist/${r.id}`} variant="outline" >Edit</Link></button>
                      <button type="button" class="btn btn-outline-dark"><Link to={`/receptionist_data/update_receptionist`} variant="outline" onClick={e => handleDelete(r.id)}>Delete</Link></button>
                    </div>
                  </CButtonGroup>
                </CTableRow>
              ))}
