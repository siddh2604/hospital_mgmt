...
const Page404 = () => {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: "http://localhost:8080/user/get_chemist_data",
      headers: {
        Authorization: token,
      },
    }).then((response) => {
  ...
  function handleDelete(id) {
    try {
      fetch('http://localhost:8080/user/delete_doctor', {
        method: 'POST',
        headers: {
          Authorization: token,
    ...
    <div className="">
      ...
      <div className="wrapper d-flex flex-column min-vh-100 bg-light">
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
          <Link to={`/create_chemist`}><button class="btn btn-primary me-md-2" type="button">Register Chemist</button></Link>
        </div>
          <CTable>
            <CTableHead>
              <CTableRow>
                ...
                <CTableRow key={i}>
                  ...
                  <CButtonGroup size="sm" role="group" aria-label="Small button group">
                    <div class="btn-group btn-group-sm" role="group" aria-label="Small button group">
                      <button type="button" class="btn btn-outline-dark"> <Link to={`/Chemist_data/update_chemist/${r.id}`} variant="outline" >Edit</Link></button>
