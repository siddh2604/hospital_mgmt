...
const Page404 = () => {
  ...
  function handleDelete(id) {
    try {
      fetch('http://localhost:8080/user/delete_medicine', {
        method: 'POST',
        headers: {
          Authorization: token,
    ...
    <div className="">
      ...
      <div className="wrapper d-flex flex-column min-vh-100 bg-light">
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
          <Link to={`/create_medicine`}><button class="btn btn-primary me-md-2" type="button">Register medicine</button></Link>
        </div>
          <CTable>
            <CTableHead>
              <CTableRow>
            ...
            <CTableBody>
              ...
              {data.map((r, i) => (
                <CTableRow key={i}>
                  <CTableDataCell>{r.content}</CTableDataCell>
                  <CTableDataCell>{r.medicine}</CTableDataCell>
                  <CButtonGroup size="sm" role="group" aria-label="Small button group">
                    <div class="btn-group btn-group-sm" role="group" aria-label="Small button group">
                      <button type="button" class="btn btn-outline-dark"> <Link to={`/medicine_data/update_medicine/${r.id}`} variant="outline" >Edit</Link></button>
