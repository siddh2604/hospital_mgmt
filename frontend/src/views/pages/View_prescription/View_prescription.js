...
const Page404 = () => {
  ...
  useEffect(() => {
    axios({
      method: "POST",
      url: "http://localhost:8080/user/get_prescription_data_by_appointment_id",
      headers: {
        Authorization: token,
      },
      data: {
          ...
          <CTable>
            <CTableHead>
              <CTableRow>
                ...
                {/* <CTableHeaderCell scope="col"><button className="btn btn-outline-success" onClzck={addTableRows} >+</button></CTableHeaderCell> */}
              </CTableRow>
            </CTableHead>
            <CTableBody>
              {data.map((r, i) => (
